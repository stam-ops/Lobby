...
import {
  ArchetypeDetailDto, ArchetypeRowDto, TournamentDetailDto, TournamentListDto, TournamentRowDto,
...
export class CatalogService {
  ...
  async tournaments(limit: number, offset: number): Promise<TournamentListDto> {
    const items = await this.dataSource.query<TournamentRowDto[]>(
      `SELECT tournamentid AS id, label, starttime AS startTime, launchstate AS launchState,
       ...
       FROM tournament ORDER BY tournamentid DESC LIMIT ? OFFSET ?`,
      [limit, offset],
    );
    const totalRow = await this.dataSource.query<{ total: number }[]>('SELECT COUNT(*) AS total FROM tournament');
