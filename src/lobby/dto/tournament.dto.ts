...
/** Résultat de la jointure tournament + tournamentarchetype */
export class TournamentDto {
  ...
  @ApiProperty({ example: 0, description: 'tournament.subscriptionstate (mis à jour par TournamentServer)' })
  ...
  @ApiProperty({ example: 0, description: 'tournamentarchetype.structuretype — ClientCodes.Tournament.StructureType' })
  structureType: number;

  @ApiProperty({ example: 0, description: 'tournamentarchetype.addonbreakindex' })
  addonBreakIndex: number;

  @ApiProperty({ example: 0, description: 'Calculé dynamiquement (non stocké directement)' })
  prizepool: number;
